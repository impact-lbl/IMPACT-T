...
#Description: PhaseOpt.py beta v. 1.0: 
...
row_start = 122 # the 123th line in the ImpactT.in
row_end = 132
...
def main():
   ...
   os.system('cp ImpactT.in ImpactT_backup.in')

   # from row_start to row_end, optimize beam elements one at a time
   row_end = fileLength('ImpactT.in')
   ...
   for i in range(row_start, row_end):
      ...
      if lines[i][0] != '!':
         ...
         if int(string.split(lines[i])[col_el]) > 100 and float((string.split(lines[i])[col_freq]).replace('d','E')) !=  0:
            ...
            # 'x' is the empty string that will be the string version of ImpactT.in. We insert -3 and -99 lines
            # after line i.
            x = ''
            ...
            if int(string.split(lines[row_restart])[1]) == 1:
               ...
               os.system('cp temp_fort.80 fort.80')
...
#            os.system('aprun -n 1 ./xmain > a')
            os.system('./ImpactTexe > a')
            ...
            end_pos = float(string.split(line)[2])
            ...
            os.system('cp fort.80 temp_fort.80')
...
def predict(zedge, prev_zedge, max_ang, prev_freq, prev_length):
   ...
   for ind in range(len(lines18)):
      ...
      if math.fabs(float(string.split(lines18[ind])[1]) - prev_zedge) < 0.01:
         ...
         diff_new1 = math.fabs(float(string.split(lines18[ind])[1]) - prev_zedge)
         # Save the time value
         t1 = float(string.split(lines18[ind])[0])
      ...
      if math.fabs(float(string.split(lines18[ind])[1]) - (prev_zedge + prev_length)) < 0.01:
         diff_new2 = math.fabs(float(string.split(lines18[ind])[1]) - (prev_zedge + prev_length))
         t2 = float(string.split(lines18[ind])[0])
         if diff_new2 < diff2:
            diff2 = diff_new2
            t2 = float(string.split(lines18[ind])[0])
...
#   os.system('aprun -n 1 ./xmain > a')
   ...
   if math.isnan(float(eng)) or math.fabs(cav_end - end_pos) > 5*1.2*10**-3:
      eng = 0.0
