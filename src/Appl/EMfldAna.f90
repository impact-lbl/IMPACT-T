...
!----------------------------------------------------------------
      module EMfldAnaclass
        ...
        interface getparam_EMfldAna
          module procedure getparam1_EMfldAna,  &
                          getparam2_EMfldAna,   &
                          getparam3_EMfldAna
        end interface
        interface setparam_EMfldAna
          module procedure setparam1_EMfldAna,  &
                           setparam2_EMfldAna, setparam3_EMfldAna
        end interface
      contains
        subroutine construct_EMfldAna(this,numseg,nmpstp,type,blength)
        implicit none
        type (EMfldAna), intent(out) :: this
        ...
        end subroutine construct_EMfldAna
   
        subroutine setparam1_EMfldAna(this,i,value)
        implicit none
        type (EMfldAna), intent(inout) :: this
        integer, intent(in) :: i
        double precision, intent(in) :: value

        this%Param(i) = value

        end subroutine setparam1_EMfldAna
        ...
        end subroutine setparam2_EMfldAna
        ...
        end subroutine setparam3_EMfldAna
        ...
        end subroutine getparam1_EMfldAna
        ...
        end subroutine getparam2_EMfldAna

        subroutine getparam3_EMfldAna(this,blength,bnseg,bmapstp,&
                                       btype)
        implicit none
        type (EMfldAna), intent(in) :: this
        ...
        end subroutine getparam3_EMfldAna
...
!        call getaxfldE_EMfldAna(t,this,ein,e1in,e2in)
        call getaxfldEfc_EMfldAna(t,this,ein,e1in,e2in)
        ...
        call rk6i_EMfldAna(h,mpstp,t0,y,18,this,Bchg,Bmass)
        ...
        sqf3=sqrt(uf)**3
        tfin = t + tau
!        call getaxfldE_EMfldAna(tfin,this,ef,e1f,e2f)
        call getaxfldEfc_EMfldAna(tfin,this,ef,e1f,e2f)
        ...
        end subroutine maplinear_EMfldAna
        ...
        tint=t
        do i=1,ns
          call intfunc1_EMfldAna(t,y,f,this,Bchg,Bmass)
          a=h*f
          yt=y+a/9.d0
          tt=t+h/9.d0
          call intfunc1_EMfldAna(tt,yt,f,this,Bchg,Bmass)
          b=h*f
          yt=y + (a + 3.d0*b)/24.d0
          tt=t+h/6.d0
          call intfunc1_EMfldAna(tt,yt,f,this,Bchg,Bmass)
        ...
        zedge = this%Param(1)
!        call getaxfldE_EMfldAna(t,this,ez1,ezp1,ezpp1)
        call getaxfldEfc_EMfldAna(t,this,ez1,ezp1,ezpp1)
        ww = this%Param(3)/Scfreq 
        theta0 = this%Param(4)*asin(1.0)/90 
        qmcc = Bchg/Bmass
...
! Here, we have only Er, Ez, and Btheta for TM mode.
        ...
        end subroutine getfld6_EMfldAna
        
        subroutine getfld6err_EMfldAna(this,pos,extfld,dx,dy,anglex,angley,&
                                    anglez)
        type (EMfldAna), intent(in) :: this
        double precision, dimension(4), intent(in) :: pos 
        double precision, intent(in) :: dx,dy,anglex,angley,anglez
...
! Here, we have only Er, Ez, and Btheta for TM mode.
        ...
        end subroutine getfld6xyz_EMfldAna
        
        subroutine getfld6xyzerr_EMfldAna(this,pos,extfld,dx,dy,anglex,angley,&
                                    anglez)
        type (EMfldAna), intent(in) :: this
        double precision, dimension(4), intent(in) :: pos 
        double precision, intent(in) :: dx,dy,anglex,angley,anglez
        ...
        end subroutine getfld6xyzerr_EMfldAna
...
! user should supply his own analytical function if needed.
        ...
        if(itype.eq.-1) then
          !alpha magnet
          call  getfldtalpha_EMfldAna(pos,extfld,this,fldata)
        else if(itype.eq.-2) then
          !traveling wave meander plates
          call getfldtTW_EMfldAna(pos,extfld,this,fldata)
        else if(itype.eq.-3) then
          !DC surface roughness fields
          call  getfldtsurfrough_EMfldTAna(pos,extfld,this,fldata)
        else if(itype.eq.10) then
          !dipole kick in Cartesian coordinate
          call getfldtdipole_EMfldAna(pos,extfld,this,fldata)
        else
          print*,"no such type of field yet",itype
          stop
        endif
        ...
        type (fielddata), intent(in) :: fldata
        double precision, dimension(6), intent(out) :: extfld
        double precision :: zedge,escale,theta0,ww,len,zz,xl,xlrep
        double precision :: ez1,ezp1,ezpp1,ezppp,f1,f1p,clite,tmpsin,tmpcos,pi
        double precision :: tmpex,tmpey,tmpez,tmpbx,tmpby,tmpbz,r2
        double precision :: c5,c6,s5,s6,zmid,z
        integer :: i
        double precision :: tt,t0,tt1,z0,zslope,vzspeed,z0t,z1,z2
        real*8 :: an,wkn
        ...
        theta0 = this%Param(4)*asin(1.0d0)/90
        an = this%Param(7)
        wkn = this%Param(8)

        !print*,"t0,t1: ",t0,tt1,tt,z0,zslope,vzspeed,escale
        ...
        tmpby = 0.0
        tmpbz = 0.0
        if(pos(3)>=zedge .and. pos(3)<=zedge+len) then
          tmpex = escale*an*wkn*exp(-wkn*(pos(3)-zedge))*&
                  sin(wkn*pos(1))
          tmpey = 0.0d0
          tmpez = escale+escale*an*wkn*exp(-wkn*(pos(3)-zedge))*&
                  cos(wkn*pos(1))
        else
        endif
        ...
        extfld(5) = extfld(5) + tmpby
        extfld(6) = extfld(6) + tmpbz
        !print*,"efield: ",extfld(1),extfld(2),extfld(3),pos(1),pos(2),&
        !        pos(3),pos(4)
        ...
        !ww = this%Param(3)*2*pi 
        !theta0 = this%Param(4)*asin(1.0)/90
        !t0 = fldata%coefey(1) !wave starting time
        !tt1 = fldata%coefey(2)-t0 !wave ending time
        !z0 = fldata%coefey(3) !wave starting location
        !zslope = fldata%coefey(4) !wave rising length
        !vzspeed = fldata%coefey(5) !wave z speed

        t0 = this%Param(3) !wave starting time
        tt = pos(4) - t0 
        tt1 = this%Param(4)-t0 !wave ending time
        z0 = this%Param(6) !wave starting location
        zslope = this%Param(7) !wave rising length
        vzspeed = this%Param(8) !wave z speed
        ...
        if(pos(3)>=zedge .and. pos(3)<=zedge+len) then
          ...
          if(tt>=0.0 .and. tt<= tt1 ) then
            z0t = z0 + vzspeed*tt
            z1 = z0t - zslope 
            z2 = z0t + zslope
            if(pos(3)>=z1 .and. pos(3)<=z2) then
              tmpey = escale/zslope*(pos(3)-z0t)
            else if(pos(3)>z2) then
              tmpey = escale 
            else
              tmpey = -escale 
            endif
        ...
        end subroutine getfldtTW_EMfldAna
